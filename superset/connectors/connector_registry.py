...
class ConnectorRegistry(object):
    ...
    def get_datasource(cls, datasource_type, datasource_id, session):
        ...
        print(session)
        print(type(session))
        print('connector_registry.py-->q0q0q0')
        print(session.query(cls.sources[datasource_type]).filter_by(id=datasource_id).first())
        print(type(session.query(cls.sources[datasource_type]).filter_by(id=datasource_id).first()))
        print('connector_registry.py-->w1w1w1')
        return (
