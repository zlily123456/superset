    ...
    def get_data(self, df):
        ...
        for name, ys in df.iteritems():
            ...
            if isinstance(name, string_types):
                ...
                series_title = ", ".join(l)
            if series_title in bar_metrics:
                d = {
                    "key": series_title,
                    "bar": True,
                    "values": [
...
class FilterBoxViz(BaseViz):
    ...
    def get_data_old(self, df):
        qry = self.query_obj()
        filters = self.form_data.get('groupby',[])
        # qry['groupby'] = filters
        df = super(FilterBoxViz, self).get_df(qry)
        d = {}
        for indx, flt in enumerate(filters):
            ctrls, is_ctrl = self.isControl(flt)
            if is_ctrl:
                ctrls_index = filters.index(ctrls)
                for row in df.itertuples(index=False):
                    print(000, row[ctrls_index])
                    print(111, row[indx])
                    if row[ctrls_index] == ctrls and row[indx] == flt:
                        data = {
                                'id': row[indx],
                                'text': row[indx],
                                'filter': flt,
    ...
    def isControl(self, flt):
        # contrls = {'region': ['country_name']}
        is_ctrl = False
        ctrls = ''
        filter_control_list = self.form_data.get('filter_control_list', {})
        if filter_control_list:
            contrls = json.dumps(filter_control_list)
            for ctrl in contrls:
                ctrleds = contrls.get(ctrl, [])
                if flt in ctrleds:
                    is_ctrl = True
                    ctrls = ctrl
        return ctrls, is_ctrl
...
class GaugeViz(BaseViz):
    ...
    viz_type = "gauge"
    verbose_name = _("Gauge View")
    credits = 'a Dataway original'
    ...
    def get_df(self, query_obj=None):
        df = super(GaugeViz, self).get_df(query_obj)
        if (
                        self.form_data.get("granularity") == "all" and
                        'timestamp' in df):
            del df['timestamp']
...
class zTreeViz(BaseViz):
    ...
    def query_obj(self):
        d = super(zTreeViz, self).query_obj()
        fd = self.form_data
        #print('order_by_cols=',fd.get('order_by_cols', []))
        if fd.get('all_columns') and (fd.get('groupby') or fd.get('metrics')):
            raise Exception(
                "Choose either fields to [Group By] and [Metrics] or "
                "[Columns], not both")
    ...
    def get_df(self, query_obj=None):
        df = super(zTreeViz, self).get_df(query_obj)
        if (
                self.form_data.get("granularity") == "all" and
                'timestamp' in df):
            del df['timestamp']
