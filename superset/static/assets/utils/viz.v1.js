...
export function bP() {
  ...
  bP.bars = function (mb) {
    ...
    function bpmap(a/*array*/, p/*pad*/, m/*min*/, s/*start*/, e/*end*/) {
      ...
      var ln = 0, lp = 0, t = d3.sum(a, function (d) {
        return d.value;
...
export function gg() {
  ...
  var def = {
    ...
    , ticks: d3.range(0, 101, 2), majorTicks: function (d) {
      return d % 10 === 0
    }
    , labelLocation: .7
  };
  ...
  gg.defs = function (svg, dg) {
    ...
    var lg1 = defsFun(defs).lG().id("vizgg1" + dg).sel();
    defsFun(lg1).stop().offset("0").stopColor(nc);
    defsFun(lg1).stop().offset("1").stopColor(d3.rgb(nc).darker(1));

    var rg1 = defsFun(defs).rG().id("vizgg2" + dg)
      .fx("35%").fy("65%").r("65%").spreadMethod("pad").sel();
    defsFun(rg1).stop().offset("0").stopColor(fc);
    defsFun(rg1).stop().offset("1").stopColor(d3.rgb(fc).darker(2));

    var rg2 = defsFun(defs).rG().id("vizgg3" + dg)
      .fx("35%").fy("65%").r("65%").spreadMethod("pad").sel();
    defsFun(rg2).stop().offset("0").stopColor(fbc);
    defsFun(rg2).stop().offset("1").stopColor(d3.rgb(fbc).darker(2));

    defsFun(defs).lG().id("vizgg4" + dg).gradientUnits("userSpaceOnUse")
      .y1("80").x1("-10").y2("80").x2("10").xlink("#vizgg1" + dg)

    var fl1 = defsFun(defs).filter().id("vizgg5" + dg).sel();
    defsFun(fl1).feFlood().result("flood").floodColor("rgb(0,0,0)").floodOpacity("0.6");
    defsFun(fl1).feComposite().result("composite1").operator("in").in2("SourceGraphic").in("flood");
    defsFun(fl1).feGaussianBlur().result("blur").stdDeviation("2").in("composite1");
    defsFun(fl1).feOffset().result("offset").dy("2").dx("2");
    defsFun(fl1).feComposite().result("composite2").operator("over").in2("offset").in("SourceGraphic");

    var fl2 = defsFun(defs).filter().x("-0.3").y("-0.3").height("1.8").width("1.8").id("vizgg6" + dg).sel();
    defsFun(fl2).feGaussianBlur().stdDeviation("2");
  }
...
export function ch() {
  ...
  function relayouts(fixedSource) {
    ...
    var m = d3.map(newchords.map(function (d) {
        return {startAngle: d.startAngle, endAngle: d.endAngle, indexsubindex: d.indexsubindex};
      })
      , function (d) {
        return d.indexsubindex;
      });
...
export function lg() {
  function lg(_) {
    ...
    _.each(function () {
      ...
      var size = lg.size(), cellHeight = rowScale.bandwidth(), cellWidth = colScale.bandwidth(),
      ...
      var legendg = g.selectAll(".legendg").data(loc, function (d) {
        return d.key;
      }).enter().append("g").attr("class", "legendg")
        .attr("transform", function (d) {
          return "translate(" + d.x + "," + d.y + ")";
        });
